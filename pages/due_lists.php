...
<div class="row" ng-controller="feedueapi" ng-model="exceltitle">
                ...
                <label class="form-label"> Date</label>
                 <div class="input-group">
                   <span class="input-group-text"><i class="fa-solid fa-calendar"></i></span>
 					<input type="text" value="<?php echo $dt;?>" id="dateRange"  autocomplete="off"  ng-model="dat_range"  class="form-control form-control-sm valid dateRange " title="Exp Date"  >
  						...
  						<table class="table table-sm table-striped table-hover table-bordered feeduecolc" cellpadding="0" cellspacing="0">
														<thead>
															<tr>
																...
																<th class="text-center">Mobile</th><th class="text-center">Type</th>
																<th >TakenDate</th>
																<th >TotalAmt</th><th >Discount</th><th >CustomerAmt</th><th >Mon/Week Amt</th><th >TotalPaid</th>
																<th >LastPaid</th>
														...
														<tbody >
																...
																<td >{{row.city}}</td>
																<td >{{row.mobile_no}}</td><td ><span ng-if='row.pay_type==1'>Weekly</span><span ng-if='row.pay_type==2'>Monthly</span></td>
																<td >{{row.tdt}}</td>
																
																
																<td >{{row.org_amount}}</td><td >{{row.conces}}</td><td >{{row.cust_amt}}</td><td >{{row.monthly_amount}}</td><td >{{row.tpaid}}</td>
																
																<td >{{row.lst_padidt}}</td>
																<td >{{row.tweeks}}</td>
																<td >{{row.term_due}}</td>
																</tr>	
														<tr><td colspan="13">Grand Tot</td><td ng-model="sub_tot" >{{sub_tot}}</td></tr>
														</tbody>
													</table>
