 ...
 $dt2=date('Y-m-d');
  ...
  if($_REQUEST['course_id']==0)$var="";
		 ...
		 if($_REQUEST['course_id']==0)
      		   ...
      		   $pay_details=$obj_db->qry("select fee_name,b.student_id,sum(is_paid_amount) as amt,b.payment_type,a.fee_id,receipt_no,date(paid_date) as dt,b.branch_id,b.course_id,d.course_name,course_hostel_id,c.user_name,e.pay_name,bank_name,cheque_num,cheque_date,receipt_cancelled,b.cancel_reason,date(cancel_date) as cdt,f.first_name,f.last_name,b.common_receipt,h.year,a.common_receipt_title from ".TABLE_FEE_TYPE." a, ".TABLE_FEE_PAYMENT." b,".TABLE_USER_DETAILS." c,".TABLE_COURSE." d,".TABLE_PAYMENT_TYPE." e,".TABLE_STUDENTDETAILS." f,".TABLE_YEAR." h where a.fee_id=b.fee_id   and b.branch_id in(".$brn_id.") $cnd and b.user_id=c.user_id and b.year_id=h.year_id and b.course_id=d.course_id and ((date(paid_date) between '".trim($dt1)."' and '".trim($dt2)."') or (date(cancel_date) between '".trim($dt1)."' and '".trim($dt2)."')) and f.student_id=b.student_id and b.payment_type=e.pay_type_id $comcnd   group by receipt_no,fee_id order by a.common_receipt asc,b.id asc ");
 					 $total_amt=0;$d=0;
					 $iscancel=0;
  ...
  foreach($cancel_paydts as $k=>$v){ 
					 ...
					 $dailycancels[$c]['receipt_no']=$v['receipt_no'];
					  $dailycancels[$c]['cancel_date']=date('d-m-Y',strtotime($v['cdt']));
					   $dailycancels[$c]['paid_date']=date('d-m-Y',strtotime($v['dt']));
					 $dailycancels[$c]['reason']=$v['cancel_reason'];
					 $c++;
  }
  
  $field_array=array("year_dts"=>$yearpay_res,"year_amts"=>$yaramt_data,"payment_types"=>$fetch_data,"extitle"=>$title,"dailyfees"=>$dailyfetds,"grdtotal"=>number_format($total_amt,2),"cancelfees"=>$dailycancels);
	echo json_encode($field_array);
}
	...
	$dt2=date('Y-m-d');
	 ...
	 if($_REQUEST['course_id']==0)$var="";
						...
						$usrcnd=" ";
						  ...
						  $usrcnd=" and a.user_id='".$_SESSION['user_id']."' ";
						
			if($_REQUEST['course_id']==0)
						 $cnd="";
						else $cnd=" and d.course_id ='".$_REQUEST['course_id']."'";
				 $pay_details=$obj_db->qry("select fee_name,b.student_id,sum(is_paid_amount) as amt,b.payment_type,a.fee_id,receipt_no,date(paid_date) as dt,b.branch_id,b.course_id,d.course_name,course_hostel_id,c.user_name,e.pay_name,bank_name,cheque_num,cheque_date,receipt_cancelled,b.cancel_reason,date(cancel_date) as cdt,f.first_name,f.last_name,b.common_receipt,h.year,a.common_receipt_title from ".TABLE_FEE_TYPE." a, ".TABLE_FEE_PAYMENT." b,".TABLE_USER_DETAILS." c,".TABLE_COURSE." d,".TABLE_PAYMENT_TYPE." e,".TABLE_STUDENTDETAILS." f,".TABLE_YEAR." h where a.fee_id=b.fee_id  $usrcnd and b.branch_id in(".$brn_id.") $cnd and b.user_id=c.user_id and b.year_id=h.year_id and b.course_id=d.course_id and d.course_type_id=3 and ((date(paid_date) between '".trim($dt1)."' and '".trim($dt2)."') or (date(cancel_date) between '".trim($dt1)."' and '".trim($dt2)."'))  and f.student_id=b.student_id and b.payment_type=e.pay_type_id $comcnd   group by receipt_no,fee_id order by a.common_receipt asc,b.id asc ");
					
				 $std_otherfes=$obj_db->qry("SELECT b.id,fee_name,date(b.enter_date) as dt,recp_no as receipt_no,transaction_no as cheque_num,course_name,b.fee_amt  as amt,b.reason,b.rec_id,b.pay_type_id,b.student_id,b.year_id,b.branch_id,c.pay_name,b.transaction_no,b.bank_name,b.is_cancel as receipt_cancelled,b.cancel_reason,date(cancel_date) as cdt,e.first_name,e.last_name,h.year,a.common_receipt_title FROM ".TABLE_STD_OTHERFEES." b,".TABLE_FEE_TYPE." a, ".TABLE_PAYMENT_TYPE." c,".TABLE_USER_DETAILS." d,".TABLE_STUDENTDETAILS." e,".TABLE_COURSE." f,".TABLE_YEAR." h where a.fee_id=b.fee_id $usrcnd and b.year_id=h.year_id and b.student_id=e.student_id $cnd  and b.user_id=d.user_id and b.pay_type_id=c.pay_type_id and f.course_type_id=3 and b.branch_id in(".$_SESSION['assign_branch_ids'].") and b.course_id=f.course_id $usrcnd and   ((date(enter_date) between '".trim($dt1)."' and '".trim($dt2)."') or (date(cancel_date) between '".trim($dt1)."' and '".trim($dt2)."'))  order by b.id desc");
                 
				 if(count($pay_details)>0)
                   $tfpaystdids=array_column($pay_details,'student_id');
				else $tfpaystdids=array();
				   if(count($std_otherfes)>0)
				   $othfepaystdids=array_column($std_otherfes,'student_id');
				 else $othfepaystdids=array();

				   $merge_tfothfepay_stids=implode(',',array_unique(array_merge($tfpaystdids,$othfepaystdids)));
				   $stdedudts=$obj_db->qry("select student_id,admission_no,reservation_no from ".TABLE_STUDENT_EDU_DETAILS." where student_id in(0".$merge_tfothfepay_stids.") group by student_id");
	 ...
	 return $v['receipt_cancelled'] == $iscancel;
					   ...
					   $dailyfetds=array();
						foreach($paydts as $k=>$v){ $i++;
							$stdeduky=array_search($v['student_id'],array_column($stdedudts,'student_id'));
					   /*$getnames=$obj_db->fetchRow("select b.first_name,b.last_name,a.roll_no,a.sec_id,a.student_status,a.admission_no from ".TABLE_STUDENT_EDU_DETAILS." a,
						".TABLE_STUDENTDETAILS." b  where a.student_id=b.student_id and a.student_id='".$pay_detailss_rows['student_id']."'");*/
						$dailyfetds[$d]['std_name']=$v['first_name'].' '.$v['last_name'];
						$dailyfetds[$d]['course_name']=$v['course_name'];
							$dailyfetds[$d]['pro_name']=$v['pro_name'];
							 $dailyfetds[$d]['branch_id']=$v['branch_id'];
							$dailyfetds[$d]['common_receipt_title']=$v['common_receipt_title']; 
							 $dailyfetds[$d]['admission_no']=$stdedudts[$stdeduky]['admission_no'];
						$dailyfetds[$d]['receipt_no']=$v['receipt_no'];
						$dailyfetds[$d]['pay_name']=$v['pay_name'];
						$dailyfetds[$d]['payment_type']=$v['payment_type'];
						 $dailyfetds[$d]['bank_name']=$v['bank_name'];
                       ...
                       $iscancel=0;
						$stdothfedts = array_filter($std_otherfes,function($v,$k) use ($iscancel){
	 return $v['receipt_cancelled'] == $iscancel;
   },ARRAY_FILTER_USE_BOTH);

   foreach($stdothfedts as $k=>$v){ $i++;
	$stdeduky=array_search($v['student_id'],array_column($stdedudts,'student_id'));
	/*$getnames=$obj_db->fetchRow("select b.first_name,b.last_name,a.roll_no,a.sec_id,a.student_status,a.admission_no from ".TABLE_STUDENT_EDU_DETAILS." a,
	 ".TABLE_STUDENTDETAILS." b  where a.student_id=b.student_id and a.student_id='".$pay_detailss_rows['student_id']."'");*/
	 $dailyfetds[$d]['std_name']=$v['first_name'].' '.$v['last_name'];
	 $dailyfetds[$d]['course_name']=$v['course_name'];
		 ...
		 $dailyfetds[$d]['common_receipt_title']=$v['common_receipt_title']; 
		  $dailyfetds[$d]['admission_no']=$stdedudts[$stdeduky]['admission_no'];
		  $dailyfetds[$d]['reservation_no']=$stdedudts[$stdeduky]['reservation_no'];
	 $dailyfetds[$d]['receipt_no']=$v['receipt_no'];
	 $dailyfetds[$d]['pay_name']=$v['pay_name'];
	 $dailyfetds[$d]['payment_type']=$v['payment_type'];
	  $dailyfetds[$d]['bank_name']=$v['bank_name'];
	...
	$iscancel=1;
	 ...
	 $iscancel=1;
						$cancel_othfepaydts = array_filter($std_otherfes,function($v,$k) use ($iscancel){
	 return $v['receipt_cancelled'] == $iscancel;
   },ARRAY_FILTER_USE_BOTH);
	
	 foreach($cancel_othfepaydts as $k=>$v){ 
						$dailycancels[$c]['std_name']=$v['first_name'].' '.$v['last_name'];
						$dailycancels[$c]['course_name']=$v['course_name'];
						$dailycancels[$c]['amount']=$v['amt'];
...
elseif($_REQUEST['action']=='opcashierwise_longtrmreprot_obk'){
...
if(count($othfepay_fids)==0)
 ...
 foreach($alfeids  as $key=>$value){
  ...
  $feidss=substr($feidss,0,-1);
     $fenames=$obj_db->qry("select * from ".TABLE_EXPENDITURE_TYPE." where exp_type_id in(0".$feidss.") order by exp_type_id asc");
   for($i=0;$i<count($fenames);$i++)
    $fee_namearr[$fenames[$i]['exp_type_id']]=$fenames[$i]['exp_name'];
...
elseif($_REQUEST['action']=='income_prejections'){
	if($_REQUEST['scholar_type']!=''){
		$cnd="and scholar_type='".$_REQUEST['scholar_type']."'";
		}
		else{
		 $cnd="";
		}
						...
						$std_tot_strength=0;
						   ...
						   $collected_tot=0;
						   $balance_tot=0;
					   $coursedetails="select a.course_name,a.course_id,a.course_model,count(*) as strength  from  ".TABLE_COURSE." a,".TABLE_COURSE_BRANCH_MAP." b,".TABLE_STUDENT_EDU_DETAILS." c where
					   a.course_id=b.course_id  and b.branch_id='".$_REQUEST['branch_id']."' and c.is_delete=0 and a.course_id=c.course_id and c.y_id='".$_SESSION['year_id']."' group by c.course_id order by a.course_id asc";
						   $coursedetails_res=$obj_db->get_qresult($coursedetails);
   
	
   
						   $courefee_details=$obj_db->qry("select b.student_id,b.first_name,b.last_name,b.mobile_no,a.course_id,e.course_name,d.fee_id,d.fee_name,sum(term_amount) as tot_amt,sum(term_income) income,sum(term_amount)-sum(term_income) as concession,sum(term_income)-sum(term_due) as collected,sum(term_due) as rbal from  
						   ".TABLE_STUDENT_EDU_DETAILS." a, ".TABLE_STUDENTDETAILS." b, ".TABLE_STUDENT_FEE." c,".TABLE_FEE_TYPE." d,".TABLE_COURSE." e where a.student_id=b.student_id    and
							is_delete=0 $cnd and a.student_id=c.student_id and c.fee_type=d.fee_id  and  c.student_id=b.student_id  and a.course_id=c.course_id and a.course_id=e.course_id and
							a.y_id='".$_SESSION['year_id']."' and a.y_id=c.y_id  and branch_id ='".$_REQUEST['branch_id']."'  group by a.course_id,a.student_id,c.fee_type order by e.coursewise_ord asc,e.course_id asc,a.student_id asc,d.fee_id asc");
   
							$arrcoursedts = array_unique(array_column($courefee_details, 'course_id'));
   $arrcoursedtsunique_arr = array_intersect_key($courefee_details, $arrcoursedts);
   
   $coursewise_feedtsarr=array();$coursewisestd_feedtsarr=array();$coursewisestd_fedttotsumarr=array();
   $feewise_dtstotsumarr=array();
   //$comfids=array(12,17,18,21,23,24);
   
   $arrstddts = array_unique(array_column($courefee_details, 'student_id'));
   $arrstddtsunique_arr = array_intersect_key($courefee_details, $arrstddts);


   $arrfeedts = array_unique(array_column($courefee_details, 'fee_id'));
   $arrarrfeedtsunique_arr = array_intersect_key($courefee_details, $arrfeedts);
   $i=0;
   $feewisestd_concelistarr=array();
   foreach($arrarrfeedtsunique_arr as $fesky=>$fesv){
	    $fid=$fesv['fee_id'];
         $feewisedts = array_filter($courefee_details,function($v,$k) use ($fid){
		   return $v['fee_id'] == $fid;
		 },ARRAY_FILTER_USE_BOTH);

		 $conceamt=0;
         $feewisedtconces = array_filter($feewisedts,function($v,$k) use ($conceamt){
		   return $v['concession'] > $conceamt;
		 },ARRAY_FILTER_USE_BOTH);
		 foreach($feewisedtconces as $stdconcky=>$stdconcev)
           $feewisestd_concelistarr[$_REQUEST['branch_id']][$fid][]=$stdconcev;
		 


		$feewise_dtstotsumarr[$i]['fee_name']=$fesv['fee_name'];
		$feewise_dtstotsumarr[$i]['stds']=count($feewisedts);
		$feewise_dtstotsumarr[$i]['totamt']=array_sum(array_column($feewisedts,'tot_amt'));
		$feewise_dtstotsumarr[$i]['conce']=array_sum(array_column($feewisedts,'concession'));
		$feewise_dtstotsumarr[$i]['income']=array_sum(array_column($feewisedts,'income'));
		$feewise_dtstotsumarr[$i]['collected']=array_sum(array_column($feewisedts,'collected'));
		$feewise_dtstotsumarr[$i]['rbal']=array_sum(array_column($feewisedts,'rbal'));
		$i++;
   }



   $brncourse_fedtsarr=array();
   $p=0;
   foreach($arrcoursedtsunique_arr as $coursek=>$coursev) {
   $coursefesdts=array();
						   $coursid=$coursev['course_id'];
	   $coursefesdts = array_filter($courefee_details,function($v,$k) use ($coursid){
		   return $v['course_id'] == $coursid;
		 },ARRAY_FILTER_USE_BOTH);
   
		 $arrcoursestddtsunique_arr = array_filter($arrstddtsunique_arr,function($v,$k) use ($coursid){
		   return $v['course_id'] == $coursid;
		 },ARRAY_FILTER_USE_BOTH);
		 $arrcoursestddtsunique_arrs=array();
		  foreach($arrcoursestddtsunique_arr as $arstdk=>$arstdv){
		   $arrcoursestddtsunique_arrs[]=$arstdv;
		  }
		 $coursefedtsarr=array();
		 foreach($coursefesdts as $coursestfk=>$coursestfv)
		 $coursefedtsarr[]=$coursestfv;
		  $arrcoursefedts = array_unique(array_column($coursefedtsarr, 'fee_id'));
   $arrcoursefedtsunique_arr = array_intersect_key($coursefedtsarr, $arrcoursefedts);
	
   $m=1;
  /* $coursewise_feedtsarr[$_REQUEST['branch_id']][$coursid][$m]="I+E+R+C";
   $m++;*/
	foreach($arrcoursefedtsunique_arr as $fidk=>$fidv){
	   if(!in_array($fidv['fee_id'],$comfids)){
	   $coursewise_feedtsarr[$_REQUEST['branch_id']][$coursid][$m]=$fidv['fee_name'];
   $m++;}
	   }
   $i=1;$j=1;
   $kv=1;
   foreach($arrcoursestddtsunique_arrs as $stdk=>$stdv){
	$coursewisestd_fedtsarr[$_REQUEST['branch_id']][$coursid][$j][$i]=$kv;
	$i++;
	$coursewisestd_fedtsarr[$_REQUEST['branch_id']][$coursid][$j][$i]=$stdv['first_name'].' '.$stdv['last_name'];
   if($_SESSION['is_mobdata_permis']==1){
   $i++;
   $coursewisestd_fedtsarr[$_REQUEST['branch_id']][$coursid][$j][$i]=$stdv['mobile_no'];
   }
   $stdid=$stdv['student_id'];
   $eachstdfesdts = array_filter($courefee_details,function($v,$k) use ($stdid){
	   return $v['student_id'] == $stdid;
	 },ARRAY_FILTER_USE_BOTH);
	 $eachstdfedtsarr=array();
	 $eachstdfedtsarr[]=array('student_id'=>'','first_name'=>'','last_name'=>'','course_id'=>'','course_name'=>'','fee_id'=>'','fee_nam'=>'','tot_amt'=>'','income'=>'','concession'=>'','collected'=>'','rbal'=>'');
	 foreach($eachstdfesdts as $stdfk=>$stdfv)
	 $eachstdfedtsarr[]=$stdfv;
   $stdcommonfes_arrlist=array();
   foreach($comfids as $comfk=>$comfid){
	 $stdcomfescrchky=array_search($comfid,array_column($eachstdfedtsarr,'fee_id'));
	 $stdcommonfes_arrlist[$comfid]=$eachstdfedtsarr[$stdcomfescrchky]['collected'];
   }
   $i++;
  /* $coursewisestd_fedtsarr[$_REQUEST['branch_id']][$coursid][$j][$i]=array_sum($stdcommonfes_arrlist);
   $i++;*/
   foreach($arrcoursefedtsunique_arr as $fidk=>$fidv){
	   $fid=$fidv['fee_id'];
   
   if(!in_array($fidv['fee_id'],$comfids)){
	   $arscrchky=array_search($fid,array_column($eachstdfedtsarr,'fee_id'));
   $coursewisestd_fedtsarr[$_REQUEST['branch_id']][$coursid][$j][$i]=$eachstdfedtsarr[$arscrchky]['collected'];
   $i++;
   }
   }

   $coursewisestd_fedtsarr[$_REQUEST['branch_id']][$coursid][$j][$i]=array_sum(array_column($eachstdfedtsarr,'collected'));
   ...
   $kv++;
		   ...
		   $coursewisetotamt=array_sum(array_column($coursefesdts,'tot_amt'));
		   $coursewiseconce=array_sum(array_column($coursefesdts,'concession'));
		   $coursewiseincome=array_sum(array_column($coursefesdts,'income'));
		   $coursewisecolcted=array_sum(array_column($coursefesdts,'collected'));
		   $coursewiserbal=array_sum(array_column($coursefesdts,'rbal'));
		   $brncourse_fedtsarr[$p]['branch_id']=$_REQUEST['branch_id'];
						   $brncourse_fedtsarr[$p]['course_id']=$coursev['course_id'];
						   $brncourse_fedtsarr[$p]['course_name']=$coursev['course_name'];
						   $brncourse_fedtsarr[$p]['strength']=count($arrcoursestddtsunique_arrs);
						   
						   $std_tot_strength=$std_tot_strength+count($arrcoursestddtsunique_arrs); 
						   
						   $amt_tot=$amt_tot+$coursewisetotamt;
						   $concession_tot=$concession_tot+$coursewiseconce;
						   $income_tot=$income_tot+$coursewiseincome;
   
						   $brncourse_fedtsarr[$p]['tot_amt']=number_format($coursewisetotamt,2);
						   $brncourse_fedtsarr[$p]['conces']=number_format($coursewiseconce,2);
						   $brncourse_fedtsarr[$p]['income']=number_format($coursewiseincome,2);
						   $brncourse_fedtsarr[$p]['collected']=number_format($coursewisecolcted,2);
						   $brncourse_fedtsarr[$p]['bal']=number_format($coursewiserbal,2);
							$collected_tot=$collected_tot+$coursewisecolcted;
						   $balance_tot=$coursewiserbal+$balance_tot;
								 
					   if($_SESSION['is_mobdata_permis']==1)
								$intilag=4;
							else $intilag=3;
					   foreach($coursewisestd_fedtsarr[$_REQUEST['branch_id']][$coursid] as $coursestk=>$coursestv){
							 $coursefescount=count($coursestv);
							 $stdgtotsum=0;
							
							 for($g=$intilag;$g<$coursefescount;$g++){
								$coursewisestd_fedttotsumarr[$_REQUEST['branch_id']][$coursid][$g]=$coursewisestd_fedttotsumarr[$_REQUEST['branch_id']][$coursid][$g]+$coursestv[$g];
						   }
					   }
   ...
   $p++;
					   ...
					   $brncourse_fedtsarr[$p]['course_name']="Grand Total";
					$brncourse_fedtsarr[$p]['strength']=number_format($std_tot_strength);
					$brncourse_fedtsarr[$p]['tot_amt']=number_format($amt_tot,2);
					$brncourse_fedtsarr[$p]['conces']=number_format($concession_tot,2);
					$brncourse_fedtsarr[$p]['income']=number_format($income_tot,2);
					$brncourse_fedtsarr[$p]['collected']=number_format($collected_tot,2);
					$brncourse_fedtsarr[$p]['bal']=number_format($balance_tot,2);
   
					   foreach($coursewisestd_fedttotsumarr[$_REQUEST['branch_id']] as $courseidk=>$courseidv){
					   //	echo '<pre>';print_r($courseidv);echo '</pre>';
						   $maxarrky= max(array_keys($courseidv))+1;
						   $coursewisestd_fedttotsumarr[$_REQUEST['branch_id']][$courseidk][$maxarrky]=array_sum($courseidv);
					   }
					  //  echo '<pre>';print_r($coursewisestd_fedttotsumarr);echo '</pre>';
					   $array_data=array("brncoursestrentharr"=>$brncourse_fedtsarr,"coursewisestdstrenthar"=>$coursewisestd_fedtsarr,'feewise_dtstotsumarr'=>$feewise_dtstotsumarr,"coursewisestd_fepaidtotarr"=>$coursewisestd_fedttotsumarr,'coursewise_feedtsarr'=>$coursewise_feedtsarr);
