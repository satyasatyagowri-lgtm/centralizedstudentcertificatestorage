...
for($j=0;$j<sizeof($sum[0]);$j++){
 ...
 $dt2=date('Y-m-d');
  ...
  if($_REQUEST['course_id']==0)$var="";
							 ...
							 /* if($_SESSION['feetypes']==1)
							   $comcnd=" and a.common_receipt in (1) ";
							 else if($_SESSION['feetypes']==2)
							   $comcnd=" and a.common_receipt in (2) ";
							 else if($_SESSION['feetypes']==3)
							   $comcnd=" and a.common_receipt in (3) ";*/
				  $paytype="select e.exp_name as pay_name, sum(is_paid_amount) as amt,b.common_receipt from ".TABLE_FEE_PAYMENT." b,".TABLE_EXPENDITURE_TYPE." e,".TABLE_COURSE." c,".TABLE_EXPENDITURE_CATEGORY." d,".TABLE_FEE_TYPE." a where e.exp_type_id=b.payment_type and d.exp_catg_id=e.exp_catg_id and b.branch_id in(".$brn_id.")  and b.fee_id=a.fee_id $comcnd AND b.course_id=c.course_id  and date(paid_date) between '".trim($dt1)."' and '".trim($dt2)."' $var and receipt_cancelled=0 group by payment_type";
			 	 ...
			 	 //$paytype_ress=$obj_db->get_qresult($paytype);
					...
					while($paytype_rowss=$obj_db->fetchArray($paytype_res)){
					 ...
					 $fetch_data[$s][$p]=number_format($total_paytyps,2);*/
					  ...
					  $year_paymens="select year,sum(is_paid_amount) as amt from ".TABLE_FEE_PAYMENT." b,".TABLE_YEAR." d,".TABLE_COURSE." c,".TABLE_FEE_TYPE." a where  
				b.branch_id in(".$brn_id.") and b.fee_id=a.fee_id $comcnd  and d.year_id=b.year_id  and b.course_id=c.course_id and date(paid_date) between '".trim($dt1)."' and '".trim($dt2)."' $var and receipt_cancelled=0 group by d.year_id order by d.year_id asc";
		 ...
		 if($_REQUEST['course_id']==0)
		   ...
		   $pay_details=$obj_db->qry("select fee_name,b.student_id,sum(is_paid_amount) as amt,a.fee_id,receipt_no,date(paid_date) as 				                      dt,b.course_id,d.course_name,course_hostel_id,c.user_name,e.exp_name as pay_name,bank_name,cheque_num,cheque_date,receipt_cancelled,b.cancel_reason,date(cancel_date) as cdt,f.first_name,f.last_name,b.common_receipt,h.year from ".TABLE_FEE_TYPE." a, ".TABLE_FEE_PAYMENT." b,".TABLE_USER_DETAILS." c,".TABLE_COURSE." d,".TABLE_EXPENDITURE_TYPE." e,".TABLE_STUDENTDETAILS." f,".TABLE_EXPENDITURE_CATEGORY." g,".TABLE_YEAR." h where a.fee_id=b.fee_id and e.exp_catg_id=g.exp_catg_id  and b.branch_id in(".$brn_id.") $cnd and b.user_id=c.user_id and b.year_id=h.year_id and b.course_id=d.course_id and date(paid_date) between '".trim($dt1)."' and '".($dt2)."' and f.student_id=b.student_id and b.payment_type=e.exp_type_id $comcnd and g.exp_catg_id=e.exp_catg_id  group by receipt_no,fee_id order by b.id asc ");
 					 $total_amt=0;$d=0;
					 $iscancel=0;
