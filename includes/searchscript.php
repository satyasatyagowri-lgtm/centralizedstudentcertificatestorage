...
$(document).on("click", "#exportExcel", function (e) {
    e.preventDefault();
    exportExcel2(); // your excel function here
});
...
$('#downloadPdf').on('click', async function(btnelement) { 
   // translateuniNames()
    const button = $(this);
...
function exportExcel() { 
    ...
    var fileName = ($("#export_datatitle").html() || "report") + ".xlsx";

    // ?? Send to Android
    if (window.Android && Android.saveExcelBase64) {
        Android.saveExcelBase64(wbout, fileName);
    } else {
...
function exportExcel2() { 
    ...
    $(".exportArea").each(function () {
        const title = $(this).data("title") || "Section";
        divData.push([title, ""]);
        ...
        $(this).find("p").each(function () {
            ...
            if (txt.includes(":")) {
                ...
                divData.push([key.trim(), value.trim()]);
            } else if (txt) {
                divData.push([txt, ""]);
            }
        });
    ...
    const divSheet = XLSX.utils.aoa_to_sheet(divData);
    XLSX.utils.book_append_sheet(wb, divSheet, "DIVs");

    /* ----------- TABLES SHEETS ------------- */
    $(".export-table").each(function (i) {
        const sheet = XLSX.utils.table_to_sheet(this);
        XLSX.utils.book_append_sheet(wb, sheet, `Table_${i + 1}`);
    });

    /* ----------- FILE NAME (SAFE) ----------- */
    const rawTitle = $('#export_datatitle').text().trim() || "report";
...
var formattedDateTime = now.getFullYear() + "-" +
    ...
    const fileName = rawTitle
        .replace(/[^\w\d]+/g, '_')
        .replace(/_+/g, '_') + formattedDateTime + ".xlsx";

    /* ----------- ANDROID vs BROWSER ----------- */

  //</br></br>  var title = $("#export_datatitle").html() || "report";



// Final filename
//var fileName = title + "_" + formattedDateTime + ".xlsx";

    if (window.Android && typeof Android.saveExcelBase64 === "function") {
        // ? Android flow (ONLY this, no writeFile)
...
/*flatpickr(".dateRange", {
  ...
  flatpickr(".dateRange", {
    ...
    dateFormat: "d-m-Y", // dd-mm-YYYY
    allowInput: true,
    minRange: 0, // âœ… allows same date as from & to
   // defaultDate: [new Date(), new Date()], // Today as both From & To
   /* onReady: function(selectedDates, dateStr, instance) {
      // Pre-fill with today â†’ today
