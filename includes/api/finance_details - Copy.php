	...
	elseif($_REQUEST['line_id']==0){
			 ...
			 if($_REQUEST['line_id']<1) 
			  $title='ALL BRANCH-['. $dt1. ' To ' . $dt2.']';
  			  ...
  			  $getcolcrep=$obj_db->qry("select a.id,line_name,a.customer_id,sum(is_paid_amount) as amt,receipt_no,date_format(str_to_date(a.paid_date,'%Y-%m-%d %H:%i:%s %p'),'%d-%m-%Y %h:%i:%s %p')  as pdt,date_format(str_to_date(a.enter_date,'%Y-%m-%d %H:%i:%s %p'),'%d-%m-%Y %h:%i:%s %p')  as edt,date_format(str_to_date(a.cancel_date,'%Y-%m-%d %H:%i:%s %p'),'%d-%m-%Y %h:%i:%s %p') as cdt,a.cancel_reason,b.user_name,d.borrow_name,customer_name,mobile_no,f.pay_type_id,f.pay_name from  ".TABLE_CUST_PAYMENTS." a,".TABLE_USER_DETAILS." b,".TABLE_LINE_NAMES." c,".TABLE_CUSTOMER_GENPAYMENTS." d,".TABLE_CUSTOMER_DTS." e,".TABLE_PAYMENT_TYPE." f where   a.pay_type_id=f.pay_type_id and a.borrow_id=d.borrow_id and e.customer_id=a.customer_id  and a.is_delete=0  and b.user_id=a.user_id $cndlin and a.line_id=c.line_id and date_format(str_to_date(paid_date,'%Y-%m-%d'),'%d-%m-%Y') between '".trim($dt1)."' and '".trim($dt2)."'  group by receipt_no,a.borrow_id order by a.id asc ");
...
$c=0;
  foreach($cancelpaydts as $cncsfpky=>$cncfpv){ 
					 ...
					 $dailycancels[$c]['amount']=$cncfpv['pamt'];
  ...
  $field_array=array("payment_types"=>$paydtsar,"extitle"=>$title,"dailyfees"=>$dailyfetds,"grdtotal"=>number_format($total_amt,2),"tdat"=>$tdat,"cancelfees"=>$dailycancels);
	echo json_encode($field_array);
}
    ...
    if($_REQUEST['branch_id']==0){
			 ...
			 if($_REQUEST['branch_id']<1) 
			  $title='ALL BRANCH-['. $dt1. ' To ' . $dt2.']';
  				 ...
  				 $fee_paymentqry=$obj_db->qry("select date(paid_date) as pdts,sum(is_paid_amount) as pamt,a.pay_type_id as pay_type_id,b.pay_name,a.borrow_id,a.user_id from ".TABLE_CUST_PAYMENTS." a,".TABLE_PAYMENT_TYPE." b  where  date_format(str_to_date(paid_date,'%Y-%m-%d'),'%d-%m-%Y') between '".trim($dt1)."' and '".trim($dt2)."' $var and is_delete=0  $lnecnd and a.pay_type_id=b.pay_type_id  group by date(paid_date),a.user_id asc,a.pay_type_id asc order by date(paid_date) asc,a.user_id asc,a.pay_type_id asc");
 ...
 $dt2=date('Y-m-d');
   ...
   if($_REQUEST['lineid']>0 && $_REQUEST['city_id']>0)
    ...
    $lnecnd=" and b.line_id='".$_REQUEST['lineid']."' ";
			...
			$count=$obj_db->fetchNum("select a.customer_id,monthlydue_amt from ".TABLE_CUSTOMER_WEEKMON_PAYYMENTS." a,".TABLE_CUSTOMER_DTS." b,".TABLE_CUSTOMER_GENPAYMENTS." c where  c.is_delete=0 and a.customer_id=b.customer_id and a.customer_id=c.customer_id and DATE_FORMAT(STR_TO_DATE(due_date, '%d-%m-%Y'), '%Y-%m-%d')<='".date('Y-m-d',strtotime(trim($_REQUEST['dat_range'])))."' and a.borrow_id=c.borrow_id and a.year_id=c.year_id and b.is_delete=0 $lnecnd and a.year_id=".$_SESSION['year_id']." group by c.borrow_id,a.customer_id having sum(monthlydue_amt)>0  ");
	      ...
	      $getcustdueslsit=$obj_db->qry("select a.customer_id,customer_name,sum(monthlydue_amt) as term_due,pay_type,sum(CASE WHEN monthlydue_amt > 0 THEN 1 ELSE 0 END) as pendweeks,count(month_week) as totweeks,a.monthly_amt,c.total_amount,c.tot_amt_updconces,(c.total_amount-c.tot_amt_updconces) as conces,(c.tot_amt_updconces-c.remain_balance) as tpaid,c.no_months,borrow_name,mobile_no,city,date_format(str_to_date(c.taken_date,'%Y-%m-%d'),'%d-%m-%Y') as tdt,c.borrow_id from ".TABLE_CUSTOMER_WEEKMON_PAYYMENTS." a,".TABLE_CUSTOMER_DTS." b,".TABLE_CUSTOMER_GENPAYMENTS." c where  $cnd  c.is_delete=0 and a.customer_id=b.customer_id and a.customer_id=c.customer_id  and a.borrow_id=c.borrow_id and a.year_id=c.year_id $lnecnd and b.is_delete=0 and a.year_id='".$_SESSION['year_id']."' and DATE_FORMAT(STR_TO_DATE(due_date, '%d-%m-%Y'), '%Y-%m-%d')<='".date('Y-m-d',strtotime(trim($_REQUEST['dat_range'])))."' group by c.borrow_id,a.customer_id having sum(monthlydue_amt)>0 order by sum(monthlydue_amt) asc ,customer_name asc  limit $pagesize, 1000");					
  ...
  $exp_rettotsums=explode(',',$_REQUEST['ret_tots']);
	$arr_fetchdata=array("feedue"=>$fetch_data,"extitle"=>$exids,"totalCount"=>$count,"fee_ids"=>$retfeids,"ret_tots"=>$trmretsub_tot,"sub_tot"=>$tot_sum,"tot_org"=>$tot_org,"tot_custamt"=>$tot_custamt,"tot_conces"=>$tot_conces,"tot_tpaid"=>$tot_tpaid,"tot_monweekdue"=>$tot_monweekdue);
 echo json_encode($arr_fetchdata);
